...
for line in lines:
    ...
    if key and indent < prev_indent:
        while len(stack) > 1:
            prev = stack.pop()
        if debug:
            print("popped {}".format(prev['node'].tag))
        if debug:
            print("adding {} to {}".format(
                prev['node'].tag, stack[-1]['node'].tag))
    ...
    if key and val:
        ...
        node.text = val.strip()
        if debug:
            print("added {} as a child of {}".format(
                node.tag, stack[-1]['node'].tag))
        prev_node = node
    ...
    # bare values should be added as text for the node at the top of the stack
        ...
        if prev_node.text:
            ...
            prev_node.text = val
        if debug:
            print("added text to {}".format(stack[-1]['node'].tag))
    if key:
        prev_indent = indent
