...
def run():
  ...
  # In this case we're converting the existing shapefile to JSON and trnaslating some coded data into a usable form in one step
  ...
  if not os.path.isfile(units_path):
    ...
    # read in the shapefile and make a new geojson, assigning attributes as we go
      ...
      with fiona.open(input_path) as units:
        for idx, unit in enumerate(units):
          ...
          print("Filling in data for {} ({} / {}, {}%)".format(
            ...
            idx, len(units), round(idx / len(units) * 100, 2)), end="\r", flush=True)
          title = unit_titles_by_nptype.get(str(unit['properties']['NPTYPE']))
          formation = None
          grouping = None
          if title:
            ...
            if re.match(r".*Sonoma Volcanics.*", title) and (formation == '' or formation == None):
