...
def make_ways_mbtiles(path):
    # Export ways into the MBTiles using different zoom levels for different
    # types
    ...
    """
    # Some weirdness: if you calll subprocess.run with a list like usually do,
    # it fails because it will pass the CONF arg like -dsco "CONF=blah" and
    # ogr2ogr does not like that. It just silently ignores that dsco. Instead,
    # I'm constructing a string version of the command and pass that with
    # shell=True some it doesn't try to do any fance string escaping
