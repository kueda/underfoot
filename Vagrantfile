...
Vagrant.configure("2") do |config|
  ...
  pgsql_mods = "grep -q -F \"#{postgresql_conf_access_for_host}\" /etc/postgresql/16/main/postgresql.conf  || echo \"#{postgresql_conf_access_for_host}\" >> /etc/postgresql/16/main/postgresql.conf\n"
  %w(vagrant ubuntu underfoot).each do |user|
    ...
    [pg_hba_access_for_local_ipv4, pg_hba_access_for_host].each do |pattern|
      pgsql_mods << "grep -q -F '#{pattern}' /etc/postgresql/16/main/pg_hba.conf || echo '#{pattern}' >> /etc/postgresql/16/main/pg_hba.conf\n"
    end
  end
